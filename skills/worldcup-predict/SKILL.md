...
Answer user-facing explanations in plain Chinese. Separate these items:
...
- Method: team strength -> goal lambdas -> Poisson score matrix -> W/D/L aggregation -> Monte Carlo tournament simulation.
- Data: state which data actually entered this run, and which data is only supported by an interface but missing trusted match-specific rows.
- Warnings: relay CLI JSON `warnings`, especially missing lineup, non-exact kickoff weather, small player sample, or Elo fallback.
